//
// Created by fatih on 08.02.2017.
//
...
namespace png
{
    ...
    {
        static_assert(sizeof...(Ts) > 0, "Given list cannot be empty!");
...
namespace png2
{
    ...
    {
        template <class ListT = png::list<>>
        class vector
        {
            using internal = ListT;

        public:
            template <class T>
            constexpr auto push_back(T)
            {
                static_assert(std::is_trivially_constructible<T>{}, "png vector is a compile time vector!");
                return vector<png::append_t<ListT, T>>{};
            }

            template <class T>
            constexpr auto push_front(T)
            {
                static_assert(std::is_trivially_constructible<T>{}, "png vector is a compile time vector!");
                return vector<png::prepend_t <ListT, T>>{};
            }

            template <class T>
            constexpr auto concat(T)
            {
                return vector<png::concat_t<T, ListT>>{};
            }

            template <int i>
            constexpr auto at(std::integral_constant<int, i>)
            {
                return typename png::type_at<i, ListT>::type {};
            }

            template <class T>
            constexpr auto index_of(T)
            {
                return png::index_of<T, ListT>::value;
            }

            constexpr int length()
            {
                return png::length_t<ListT>();
            }

            constexpr auto front()
            {
                return at(std::integral_constant<int, 0>{});
            }

            constexpr auto tail()
            {
                return vector<png::tail_t<ListT>>{};
            }

            template <class F>
            constexpr void for_each(F fun)
            {
                png::for_each(ListT{}, fun);
            }
        };
